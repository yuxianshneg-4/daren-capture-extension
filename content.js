...
(function () {
  ...
  function applyDefaults() {
    // 建联进度是从「微信号」推导的，必须每次都重算：首扫时微信号还没抓到，
    // 若按“有值就不动”会先把「站内邀约」定死，之后抓到微信号也改不回来。
    // 只有用户手动选过（edited）才以他的选择为准。
    if (!edited.has('建联进度')) {
      const hit = pickOption('建联进度', current['微信号'] ? '微信号' : '站内邀约');
      if (hit) current['建联进度'] = hit;
    }
    const rules = [
      ['合作模式', () => '纯佣'],
      ['合作进度', () => '建联中']
  ...
  function render(debug) {
    ...
    for (const f of FIELDS) {
      ...
      if (f.key === '建联进度') {
        addHint(current['微信号']
          ? '已抓到微信号，自动填「微信号」；手动改过则以你的选择为准'
          : '暂未抓到微信号，自动填「站内邀约」；抓到后会跟着改成「微信号」');
      }
      if (f.key === '登记人') {
